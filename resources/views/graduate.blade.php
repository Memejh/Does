            ...
            <div class="row">
                <div class="col-md-12">
                    <ul class="nav nav-tabs" role="tablist">
                        ...
                        ?>
                      <?php
                      $news = DB::table('news_cat')->orderby('order','asc')->orderby('last_date','desc')->get();
                     foreach($news as $new)
                     {
                          if ($new->type_news_cat == 3){
                            tabsPanels($new->name_th,$new->id,$num);
                          }else{
                            continue;
                      }
                   }tabsPanels(__("u"), "http://www.reg.up.ac.th",$num);
                      ?>
...
function tabsPanels($h1, $tabid,$num) {
      ...
      if($tabid == 0){echo '<li class="" ><a class="nav-link mbr-fonts-style display-7" style=""  href="http://www.reg.up.ac.th" >
          ' . $h1 . '</a></li>';}
      else{
      echo '<li class="" ><a class="nav-link mbr-fonts-style  display-7 '.$active.'" style=""  href="'.url('graduate').'" >
      ' . $h1 . '</a></li>';}
}
?>
